...
import { deliveryWindow } from './delivery-eta.js';
 ...
 *
 * The delivery window lives here for the same reason rather than in a second
 * writer of its own. It is set on the move to confirmed and nowhere else, so
 * it is a side effect of a transition exactly as the restock is, and it has to
 * land in the same transaction: a window written by a separate UPDATE would
 * survive a move the guard refused, and the customer would be shown an arrival
 * date for an order that never left the status it was in.
 *
 * ---------------------------------------------------------------------------
...
/**
 * The statuses a customer may cancel out of on their own, without asking.
 *
 * Narrower than what the transition table allows, and deliberately so: LEGAL
 * says which moves are *coherent*, this says which ones a customer is trusted
 * to make unsupervised. They are different questions and collapsing them would
 * hand the wrong answer to one of the two callers.
 *
 * `shipped` is the line, and it is drawn there because that is where a real
 * person starts carrying a real parcel. Cancelling then does not undo the
 * delivery run — it strands a driver at a door with goods nobody will pay for,
 * on a shop where payment only happens at that door. Past this point the
 * customer asks and a human answers, which is what the refund request already
 * does.
 *
 * Exported rather than inlined in the route because the order page has to make
 * the same decision to know which control to render, and a page that offers a
 * Cancel button the API then refuses is worse than one that never offered it.
 */
export const SELF_CANCELLABLE = ['new', 'confirmed'];

export const canSelfCancel = status => SELF_CANCELLABLE.includes(status);
...
export async function transition({ orderId, to, actor = 'system', note = '' }) {
  ...
  /*
   * The delivery window, if this is the move that earns one.
   *
   * It belongs to `confirmed`: that is the call where the shop finds out the
   * order is real and the address is right, and it is the first moment it can
   * honestly promise anything. `shipped` is here as well because new ->
   * shipped is a legal move — an admin who packs a phone order without
   * pressing Confirm first — and an order that took that path would otherwise
   * carry no date at all.
   *
   * This is the one read outside the batch, and it is worth being clear about
   * what that does and does not risk. It is NOT a check-then-act on the status:
   * the guard below still decides whether the move happens, against the live
   * row, and if it refuses then the whole batch rolls back and these dates are
   * never written. All this read decides is which promise to put in the
   * statement. The worst a stale answer can produce is a window computed from
   * a city the customer changed a moment ago, which is a day out, not a
   * corrupted order.
   *
   * The alternative was to put the SLA table in SQL so the window could be
   * derived inside the UPDATE. That buys atomicity nothing needs and costs the
   * one thing that matters here: the table would then exist twice, in two
   * languages, and the copy the tests exercise would not be the copy that runs.
   */
  let eta = null;
  if (to === 'confirmed' || to === 'shipped') {
    const [row] = await sql`SELECT city FROM orders WHERE id = ${id}`;
    if (row) eta = deliveryWindow(row.city);
  }
   ...
   *           It also stamps the delivery window, which is why the window is
   *           set from in here rather than by a second UPDATE afterwards: a
   *           move that the guard refuses must not leave a promise behind on
   *           an order that did not move, and a separate statement outside the
   *           batch would do exactly that. COALESCE and not an assignment,
   *           because a column that is already filled is a date the customer
   *           has been shown and must not be quietly moved; and when `eta` is
   *           null — every move that is not a confirm or a ship — COALESCE of
   *           the column with NULL is the column, so those moves leave it
   *           alone without needing a CASE of their own.
   ...
   * parameter handed to ANY(). Inside `moved` the status parameters sit in a
   * SET clause and a comparison against a typed column, so those infer and are
   * left alone — but the two dates do not: they are arguments to COALESCE, and
   * the driver sends every parameter as text, so an uncast one is a string
   * being COALESCEd with a date column. ::date on both, and the NULL case
   * needs it most, since a bare NULL has nothing at all to infer from.
   */
  const move = sql`
    WITH before AS (
      ...
      UPDATE orders o
         SET status = ${to},
             ...
             expected_from = COALESCE(o.expected_from, ${eta?.from ?? null}::date),
             expected_to   = COALESCE(o.expected_to,   ${eta?.to ?? null}::date)
        FROM before b
  ...
  if (to === 'cancelled') {
    ...
    /*
     * And give the customer their per-customer redemption back.
     *
     * The counter above is the shop's total; this is the row that says which
     * person spent it, and the two have to move together or they stop
     * describing the same event. A cancelled order that left its redemption
     * behind would lock that customer out of a code they never actually used —
     * which is the same unfairness as a cancelled order that kept its
     * used_count, one order further down.
     *
     * Deleted rather than flagged. offer_redemptions exists to be counted, and
     * a cancelled-but-present row would mean every count in the checkout guard
     * needed a join to orders to know which rows were live. The order timeline
     * already records that the cancellation happened, which is where that
     * history belongs.
     *
     * Unguarded, exactly like the restock above it and for the same reason: the
     * guard is the status move at the head of this batch, `cancelled` is not a
     * status this can be reached from twice, and an order that carried no
     * coupon simply matches nothing. The slot numbers left behind are not
     * reused - see the redemption insert in app/api/order/route.js, which takes
     * max(slot) + 1 rather than a count precisely so that this delete cannot
     * strand a later order on a duplicate.
     */
    stmts.push(sql`DELETE FROM offer_redemptions WHERE order_id = ${id}`);
  }
