...
export async function transition({ orderId, to, actor = 'system', note = '' }) {
   ...
   *           BEFORE the UPDATE below — which is what the audit row needs and
   *           what the guard tests against.
   *   moved   applies the change, but only from a status the move is legal
   *           from. Matches nothing if the move is illegal, if the order does
   *           not exist, or if somebody else moved it first.
  ...
  const move = sql`
    WITH before AS (
      ...
      UPDATE orders o
        ...
        FROM before b
       WHERE o.id = b.id AND b.was = ANY(${from}::text[])
