...
export function renderMarkdown(md) {
  ...
  for (const line of String(md ?? '').split(/\r\n|\r|\n/)) {
    ...
    if (heading) {
      flushPara();
      closeList();
      // "##" is an h2. The old rule bumped every heading down a level "so it
      // never competes with the page's own h1" — but an h2 under an h1 is the
      // document outline working as intended, not competition. The bump meant
      // article bodies could not emit an h2 at all, which cost real structure
      // on the pages that most need it. "#" is still bumped to h2: an article
      // body must never introduce a second h1.
