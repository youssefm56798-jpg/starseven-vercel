...
/**
 * The one reader of SESSION_SECRET. Everything that needs the secret comes
 * through here, so the length rule is stated once rather than in each caller -
 * csrfToken() used to read process.env directly and so skipped this check.
 */
function secretRaw() {
...
/** The cookie that gives an anonymous visitor a CSRF token of their own. */
export const CSRF_COOKIE = 's7_csrf';
...
export async function pendingAdminId() {
  try {
    ...
    if (!token) return null;
    const { payload } = await jwtVerify(token, secret(), { audience: 's7-2fa', algorithms: ['HS256'] });
...
/**
 * CSRF: a form token that is a MAC over a value only this visitor's browser
 * holds. An attacker on another origin can make the browser send its cookies
 * but cannot read them, so cannot produce the matching token.
 *
 * Two things here were wrong and are worth naming, because both were invisible.
 *
 * The token used to be SHA-256 of (cookie + SESSION_SECRET). For a signed-in
 * admin that works. For a visitor with no cookie the input was the literal
 * 'anon' plus the secret, so every anonymous visitor was served the SAME 64 hex
 * characters - a value anyone could fetch with curl. That is not a CSRF token,
 * and it stood in front of /admin/login, /admin/forgot and /admin/reset. It was
 * also a hash of the secret published to unauthenticated callers: the secret
 * signs every session JWT and derives the key that seals every TOTP secret, so
 * a weak one would have been open to an offline crack from a single GET.
 *
 * So: anonymous visitors now get CSRF_COOKIE, thirty-two random bytes minted by
 * the middleware, and the token is a MAC over that - different for every
 * visitor, and worth something. And the MAC is HMAC under a key derived by
 * domain separation from the secret rather than a bare hash of it, so the value
 * on the page is not a known-prefix hash of the signing key any more.
 */
async function csrfKey() {
  const base = await crypto.subtle.digest(
    'SHA-256',
    new TextEncoder().encode('s7-csrf-v1:' + secretRaw()),
  );
...
export async function csrfToken() {
  const jar = await cookies();
  // Signed-in first, then the half-finished login, then the anonymous cookie.
  // 'anon' remains only as the answer when no cookie of any kind arrived, which
  // means the request cannot be a browser that just followed a link here.
  const seed =
    jar.get(COOKIE)?.value ||
    jar.get(PENDING_COOKIE)?.value ||
    jar.get(CSRF_COOKIE)?.value ||
    'anon';
  const sig = await crypto.subtle.sign('HMAC', await csrfKey(), new TextEncoder().encode(seed));
  return Array.from(new Uint8Array(sig)).map(b => b.toString(16).padStart(2, '0')).join('');
}

export async function csrfOk(sent) {
  /*
   * No seed of any kind means refuse, rather than accept the constant.
   *
   * csrfToken() falls back to the literal 'anon' when no cookie arrived, which
   * makes the token a per-deployment constant - the exact defect this whole
   * mechanism was rewritten to remove, surviving as the default branch of the
   * function that removed it. It is not reachable today: the middleware seeds
   * s7_csrf onto the request headers before the first /admin render, so a caller
   * that reaches a form has a cookie by definition.
   *
   * "Not reachable today" is a statement about the middleware, though, and this
   * is the verifier. A verifier whose fallback is to accept a value anyone can
   * compute is the wrong shape regardless of who currently protects it, so the
   * absence of a seed is now a no. Rendering is untouched - csrfToken() still
   * returns a string, so a form still draws.
   */
  const jar = await cookies();
  const seeded = jar.get(COOKIE) || jar.get(PENDING_COOKIE) || jar.get(CSRF_COOKIE);
