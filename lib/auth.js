...
/**
 * The current session epoch for an admin, or null if there is no such admin.
 *
 ...
 */
const readEpoch = cache(async id => unstable_cache(
  async () => {
    const rows = await sql`SELECT session_epoch FROM admins WHERE id = ${id}`;
    return rows.length ? Number(rows[0].session_epoch) : null;
  },
...
export async function currentAdmin() {
  try {
    ...
    const epoch = await readEpoch(Number(payload.id));
    if (epoch === null || epoch !== payload.ep) return null;
