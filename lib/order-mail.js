...
const refPill = ref =>
  `<div style="display:inline-block;background:#12100B;color:#FFFDF8;font-weight:900;
    padding:8px 16px;border-radius:99px;font-size:14px;margin-bottom:16px;">${esc(formatRef(ref))}</div>`;
...
export function tplOrderEdited(order, items, lang, trackUrl = '') {
  ...
  const body = ar
    ? `<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">عدّلنا أوردرك</h1>
       <p style="margin:0 0 8px;color:#6E6A60;">ده أوردر <b dir="ltr">${esc(formatRef(order.ref))}</b> بعد التعديل. لو فيه أي حاجة هنا مش زي ما اتفقنا، قولنا فوراً.</p>`
    : `<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">We have updated your order</h1>
       <p style="margin:0 0 8px;color:#6E6A60;">This is order <b dir="ltr">${esc(formatRef(order.ref))}</b> as it now stands. If anything here is not what you expected, tell us straight away.</p>`;
...
export function tplOrderLink(order, lang, trackUrl) {
  ...
  const body = ar
    ? `<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">لينك متابعة أوردرك</h1>
       <p style="margin:0 0 8px;color:#6E6A60;">ده لينك جديد لأوردر <b dir="ltr">${esc(formatRef(order.ref))}</b> — بيفتح الأوردر ده بس.</p>
    ...
    : `<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">Your order tracking link</h1>
       <p style="margin:0 0 8px;color:#6E6A60;">A new link for order <b dir="ltr">${esc(formatRef(order.ref))}</b> — it opens that order and nothing else.</p>
