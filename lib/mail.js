...
export function tplOrder(order, items, lang, trackUrl = '') {
  ...
  const ref = `<div style="display:inline-block;background:#12100B;color:#FFFDF8;font-weight:900;
    padding:8px 16px;border-radius:99px;font-size:14px;margin-bottom:16px;">${esc(formatRef(order.ref))}</div>`;
...
export function tplOrderAdmin(order, items) {
  ...
  const inner = `<h1 style="font-size:22px;margin:0 0 12px;font-weight:900;">أوردر جديد — ${esc(formatRef(order.ref))}</h1>
