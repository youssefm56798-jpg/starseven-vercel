...
const deliveryBlock = (order, rtl) => {
  ...
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
   ...
   <tr><td style="padding:18px 0 0;">
     ...
     ${where ? `<div style="margin-top:5px;font-weight:700;">${esc(where)}</div>` : ''}
     ${order.phone
 ...
 *
 * Kept deliberately plain. This one is read on a phone, standing up, while
 * someone decides whether to ring the customer now or after lunch — so it is a
 * list of facts and a tap-to-call link, and nothing here is trying to look like
 * anything.
 */
export function tplOrderAdmin(order, items) {
  const rows = items.map(i => `<li>${esc(i.name)} × ${i.qty} — ${amt(i.price * i.qty)}</li>`).join('');
  ...
  const wa = `https://wa.me/2${String(order.phone).replace(/\D/g, '').replace(/^0/, '')}`;

  const inner = `${eyebrow('أوردر جديد')}${title(esc(formatRef(order.ref)))}
    <p style="margin:0 0 6px;"><b>${esc(order.name)}</b> — <a href="${wa}" style="color:${C.red};font-weight:700;">${esc(order.phone)}</a></p>
    <p style="margin:0 0 6px;color:${C.grey};">${esc(order.address)} — ${esc(order.city)}</p>
    ${order.notes ? `<p style="margin:0 0 6px;color:${C.grey};">ملاحظات: ${esc(order.notes)}</p>` : ''}
    <ul style="margin:14px 0;padding-inline-start:18px;">${rows}</ul>
    <p style="font-size:19px;font-weight:900;">الإجمالي: ${amt(order.total)}</p>`;

  return [`أوردر جديد ${formatRef(order.ref)} — ${Math.round(order.total)} ${site.currency}`, shell(inner, 'ar')];
}
