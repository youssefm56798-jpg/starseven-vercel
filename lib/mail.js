...
export function tplOrder(order, items, lang) {
  const ar = lang === 'ar';
  ...
  const inner = ar
    ? `${ref}<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">استلمنا طلبك ✓</h1>
       ...
       <p style="margin:22px 0 0;color:#6E6A60;font-size:13px;">العنوان: ${esc(order.address)} — ${esc(order.city)}</p>`
    : `${ref}<h1 style="font-size:23px;margin:0 0 10px;font-weight:900;">Order received ✓</h1>
       ...
       <table width="100%" style="font-size:14px;">${rows}${totals}</table>
       <p style="margin:22px 0 0;color:#6E6A60;font-size:13px;">Address: ${esc(order.address)} — ${esc(order.city)}</p>`;
