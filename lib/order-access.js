...
export async function orderFor(ref, token) {
  ...
  const rows = await sql`
    SELECT id, ref, name, phone, address, city, notes, lang, email,
      ...
      FROM orders
     WHERE access_hash = ${await sha256(token)} AND access_hash <> ''
    UNION ALL
    SELECT o.id, o.ref, o.name, o.phone, o.address, o.city, o.notes, o.lang, o.email,
           ...
           o.refund_requested_at, o.refund_reason, o.cancelled_at,
           o.expected_from::text AS expected_from,
           o.expected_to::text   AS expected_to,
 ...
 *
 * The photograph is joined from the catalogue by SKU rather than stored on the
 * line: a receipt records the name and the price a customer agreed to, and the
 * picture is allowed to follow whatever the shop uploads later. LEFT JOIN, so a
 * line whose product was retired - or an old row with no SKU - still renders,
 * just without one.
 */
export async function itemsFor(orderId) {
  const rows = await sql`
    SELECT i.sku, i.name, i.price, i.qty, COALESCE(p.image, '') AS image
      FROM order_items i
      LEFT JOIN products p ON p.sku = i.sku
     WHERE i.order_id = ${orderId} ORDER BY i.id`;
