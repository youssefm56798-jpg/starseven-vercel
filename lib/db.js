...
export function ipBucket(ip) {
  const s = String(ip || '');
  if (s.includes(':')) {
    // The routing prefix is the first four hextets - a /64.
    const parts = s.split('%')[0].split(':');
    return parts.slice(0, 4).join(':') + '::/64';
  }
  const o = s.split('.');
  // Only coarsen a genuine dotted-quad. rateOk is also called with non-IP keys
  // (the login-account limiter passes an email), and an email with four
  // dot-separated pieces must pass through untouched rather than be rewritten
  // into a bogus /24.
  const isV4 = o.length === 4 && o.every(p => /^\d{1,3}$/.test(p) && Number(p) <= 255);
  return isV4 ? `${o[0]}.${o[1]}.${o[2]}.0/24` : s;
}
