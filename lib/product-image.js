...
export function emailImageUrl(value, base) {
  ...
  const path = s.replace(/^\/+/, '').toLowerCase();
  // The catalogue, or one of the eight launch shots that still live one level
  // up at assets/wax-*.webp and assets/gel-*.webp. Both get a mail copy from
  // scripts/gen-email-images.mjs; the first cut of this only knew the
  // catalogue and mailed the best-selling wax with no picture.
  const launch = /^assets\/((?:wax|gel)-[a-z]+)\.webp$/.exec(path);
  if (!HAS_VARIANTS.test(path) && !launch) return '';

  const stem = launch ? launch[1] : path.replace(/^assets\/catalog\//, '').replace(/\.webp$/, '');
