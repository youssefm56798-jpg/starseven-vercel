 ...
 */
/**
 * Push a catalogue change out to the storefront now, instead of in a minute.
 *
 * ---------------------------------------------------------------------------
 * The problem this fixes
 *
 * Every storefront route carries `revalidate = 60`, and /api/products adds a
 * CDN header on top of it. That is right for a catalogue that changes a few
 * times a week: visitors get a prerendered page from the edge instead of a
 * database round trip.
 *
 * What it also meant is that NOTHING the admin did showed up straight away. An
 * owner who corrected a price, hid a product or fixed a photograph reloaded the
 * shop, saw the old value, and had no way to tell whether the save had worked
 * or the page was stale. The usual next move is to save it again, which is how
 * an edit gets applied twice.
 *
 * Worse than the confusion: a product taken off sale stayed buyable at its old
 * address for up to a minute, and a corrected price stayed wrong for the same
 * window. On a shop that takes cash at the door, a stale price is a number a
 * driver tries to collect.
 *
 * So every write in this module ends here, and the affected pages are rebuilt
 * on the next request rather than when the window happens to expire.
 *
 * ---------------------------------------------------------------------------
 * Why it is this blunt
 *
 * `layout` rather than a list of individual URLs, and both language trees. One
 * product appears on the home page, the shop grid, its own page, the kind
 * filter it belongs to, the sitemap and the JSON the storefront falls back to -
 * and the SET of those pages changes with the edit itself, because featuring a
 * product, changing its `kind` or archiving it all move it between them.
 * Working out the minimal set would be a second copy of the routing rules,
 * kept in step by hand, and being wrong about it is silent.
 *
 * Rebuilding more than strictly necessary costs one render each on a catalogue
 * of sixty products, a few times a week. Being subtle here buys nothing worth
 * having.
 *
 * ---------------------------------------------------------------------------
 * After the write, never before
 *
 * The same ordering rule as bumpSessionEpoch in lib/session-epoch.js:
 * invalidating first leaves a window in which a request can refill the cache
 * from the row as it was, and the stale value then survives until the next
 * edit.
 *
 * The dynamic import is what keeps this module runnable outside a Next request
 * - scripts/verify-product-admin.mjs exercises every function in here against a
 * real Postgres with no server around it, and a top-level `next/cache` import
 * would break that. The catch is wide for the same reason: the row is written,
 * and a cache that could not be invalidated is a stale page for a minute, not a
 * failed save. It must never turn a successful edit into an error.
 */
async function published() {
  try {
    ...
    console.error('[s7] catalogue cache not invalidated:', e?.message || e);
  }
}
...
export async function toggleActive(id) {
  ...
  if (!row) return { ok: false, reason: 'archived' };
  await published();
...
export async function toggleFeatured(id) {
  ...
  if (!row) return { ok: false, reason: 'archived' };
  await published();
  return { ok: true, featured: row.featured };
}
