...
-- ---------------------------------------------------------------------------
--  The colognes are 200ml, not 180
--
--  The catalogue page prints "200 مل" in its header and the bottles in the
--  photograph carry "200ML" on the label. The rows said 180 in size_ml, in both
--  names and in both subtitles, so the page contradicted the product in the
--  hand of the customer - and the Arabic copy written for these says 200.
--
--  One statement per variant, each guarded on the English name it is replacing,
--  so a re-run is a no-op and a name edited in the admin is left alone.
--
--  The SLUGS still read cologne-180-*. Those are URLs, they are in the sitemap
--  that was submitted to Search Console, and changing them is a redirect
--  decision rather than a copy one. Left for the shop to call.
-- ---------------------------------------------------------------------------
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - AQUA',
       name_en = 'After-Shave Cologne 200ml — Aqua',
       sub_ar  = '200 مل · أكوا',
       sub_en  = '200ml · Aqua'
 WHERE sku = 'S7-C180-AQUA' AND name_en = 'After-Shave Cologne 180ml — Aqua';
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - Echo',
       name_en = 'After-Shave Cologne 200ml — Echo',
       sub_ar  = '200 مل · إيكو',
       sub_en  = '200ml · Echo'
 WHERE sku = 'S7-C180-ECHO' AND name_en = 'After-Shave Cologne 180ml — Echo';
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - ESSENCE',
       name_en = 'After-Shave Cologne 200ml — Essence',
       sub_ar  = '200 مل · إيسنس',
       sub_en  = '200ml · Essence'
 WHERE sku = 'S7-C180-ESSENC' AND name_en = 'After-Shave Cologne 180ml — Essence';
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - Fresh',
       name_en = 'After-Shave Cologne 200ml — Fresh',
       sub_ar  = '200 مل · فريش',
       sub_en  = '200ml · Fresh'
 WHERE sku = 'S7-C180-FRESH' AND name_en = 'After-Shave Cologne 180ml — Fresh';
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - Magic',
       name_en = 'After-Shave Cologne 200ml — Magic',
       sub_ar  = '200 مل · ماجيك',
       sub_en  = '200ml · Magic'
 WHERE sku = 'S7-C180-MAGIC' AND name_en = 'After-Shave Cologne 180ml — Magic';
UPDATE products
   SET size_ml = 200,
       name_ar = 'كولونيا بعد الحلاقة نيو ستار سفن 200 ملل - SENSE',
       name_en = 'After-Shave Cologne 200ml — Sense',
       sub_ar  = '200 مل · سينس',
       sub_en  = '200ml · Sense'
 WHERE sku = 'S7-C180-SENSE' AND name_en = 'After-Shave Cologne 180ml — Sense';

-- ---------------------------------------------------------------------------
--  The beeswax cream gel said "beeswax and beeswax"
--
--  The catalogue copy was written from a template of the shape "with beeswax
--  and {extract}", and for the one variant whose extract IS beeswax it rendered
--  the ingredient twice - in the body and in the highlight, in both languages.
--  Guarded on the English body, which is the phrase being replaced.
-- ---------------------------------------------------------------------------
UPDATE products
   SET long_ar       = 'كريم جل بشمع العسل. يمنح ثباتاً متوسطاً يدوم طوال اليوم مع كثافة ولمعان يدومان طويلاً.

يحتوي على شمع العسل لترطيب وتقوية الشعر. التركيبة مائية، فبتتغسل بسهولة وتقدر تعيد تصفيف شعرك في أي وقت من غير ما تسيب بقايا.',
       long_en       = 'A cream gel with beeswax. Medium hold that lasts all day, with lasting thickness and shine.

Beeswax to moisturise and strengthen the hair. The formula is water-based, so it washes out easily and you can restyle any time without residue.',
       highlights_ar = replace(highlights_ar, 'بشمع العسل وشمع العسل', 'بشمع العسل'),
       highlights_en = replace(highlights_en, 'Beeswax and beeswax', 'Beeswax')
 WHERE sku = 'S7-CG250-BEESWA' AND long_en LIKE '%beeswax and beeswax%';
