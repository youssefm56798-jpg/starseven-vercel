 ...
 *
 * This body used to sit in app/brand/page.js and take its language from
 * `searchParams`, because middleware rewrote /en/brand onto /brand?lang=en and
...
/** Title, description and hreflang alternates for one language of the page. */
export function brandMeta(lang) {
  const ar = lang !== 'en';
  return {
    ...
    alternates: alternatesForLang('/brand', ar ? 'ar' : 'en'),
  };
}
...
export default async function BrandView({ lang }) {
  const ar = lang === 'ar';
