...
| **Admin Server Actions** | A Server Action is a POST endpoint with no gate of its own, so every one of them calls `requireAdmin()` and then `csrfOk()` before it writes anything. A test walks the source, finds the actions rather than being told where they are, and fails if any is missing either check or writes before it checks. | `app/admin/**`, `tests/admin-actions.test.mjs` |
| **Malicious file upload** | The product photograph is accepted on the bytes alone: the format and the media type are read from the file signature, the dimensions come out of the header before anything decodes, and there is a 3 MB cap plus dimension and aspect bounds. The filename, the extension and the browser's `Content-Type` are all discarded — the stored name is built from the SKU plus eight random bytes under a fixed prefix, so nothing can escape it or overwrite another product. SVG is refused because it can carry script. | `lib/image-file.js`, `lib/blob.js` |
| **Untrusted image references** | `products.image` is rendered as an `<img src>` and published in the JSON-LD, so it is validated to exactly two shapes on write: a path under `public/assets` with no traversal, or an https URL on the Vercel Blob host. `imageUrl()` prefixes anything else with a slash, so even a value that predates the validation resolves to a dead path rather than to a scheme the browser will act on. | `lib/product-image.js` |
| **CSS injection through a colour** | `products.color` is interpolated into the `--c` custom property on the storefront. The admin form refuses anything that is not six hex digits. | `lib/product-form.js` |
